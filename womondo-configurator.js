...
(() => {
  ...
  // ✅ Transport fee codes (PRIMARY should be your real code)
  ...
  // ✅ Webflow textarea name for summary
  ...
  // -----------------------------
  // AUTO TRANSPORT FEE (DO0101 primary)
  // -----------------------------
  ...
  function pickExistingTransportCode(map) {
    if (!map) return null;
    if (map.has(TRANSPORT_PRIMARY)) return TRANSPORT_PRIMARY;
    if (TRANSPORT_FALLBACK && map.has(TRANSPORT_FALLBACK)) return TRANSPORT_FALLBACK;
    return null;
  }

  function getTransportGross(map, code) {
    if (!map || !code) return 0;
    const g = map.get(up(code));
  ...
  function step1SelectedHasLengthCode() {
    ...
    // 2) attribute/code scan inside selected card
  ...
  function syncAutoTransportFee() {
    ...
    if (!shouldAdd) {
      // remove any previous fee entries
      Object.keys(autoFees).forEach(k => {
        if (k === TRANSPORT_PRIMARY || k === TRANSPORT_FALLBACK) delete autoFees[k];
      });
      return;
    }

    const codeToUse = pickExistingTransportCode(map) || TRANSPORT_PRIMARY; // if not found, still send primary (price may be 0)
    const gross = getTransportGross(map, codeToUse);

    // store using the actual code we will send
    autoFees = {
      ...autoFees,
      [codeToUse]: {
    ...
    // clean the other transport code key if both exist
    if (codeToUse !== TRANSPORT_PRIMARY) delete autoFees[TRANSPORT_PRIMARY];
    if (codeToUse !== TRANSPORT_FALLBACK) delete autoFees[TRANSPORT_FALLBACK];

    if (DEBUG) {
      console.log('[TRANSPORT dbg]', {
        shouldAdd,
        currentCountryColKey,
        primary: TRANSPORT_PRIMARY,
        fallback: TRANSPORT_FALLBACK,
        existsPrimary: !!map?.has?.(TRANSPORT_PRIMARY),
        existsFallback: !!map?.has?.(TRANSPORT_FALLBACK),
        codeToUse,
        gross
      });
    }
  ...
  function collectSelectedItemsCodes() {
    ...
    // Auto transport fee: send the actual stored fee code (already chosen)
    if (autoFees && Object.keys(autoFees).length) {
      Object.values(autoFees).forEach(fee => {
        // only include transport-like fee entries
        if (fee?.name === TRANSPORT_LABEL && fee?.code) {
          items.push({ step: 'FEE', title: TRANSPORT_LABEL, code: fee.code, qty: 1 });
        }
      });
    }
  ...
  function buildPayload() {
    ...
    const rawCodes = [baseMoCode, ...items.map(i => i.code)]
      .filter(Boolean)
  ...
  // -----------------------------
  // FORM: write summary + hidden payload fields
  // -----------------------------
  ...
  function writeSummaryAndPayloadToForm(form) {
    try {
      ...
      const summary = payload?.note || buildSummaryText();
      ...
      setFieldValue(form, 'payload_full', JSON.stringify(payload));
  ...
  function normalizePayload(raw, form) {
    ...
    if (raw && Array.isArray(raw.mo_codes)) {
      if (!raw.note) {
        raw.note = (form?.querySelector(`[name="${CSS.escape(SUMMARY_TEXTAREA_NAME)}"]`)?.value || '').trim();
      }
  ...
  function bindFormJsonOnlyWebhookAndFields() {
    ...
    // ensure hidden fields exist inside form
    ['payload_json', 'payload_full', 'country_col', 'total_gross'].forEach(name => setFieldValue(form, name, ''));
    ...
    function fire() {
      try {
        ...
        if (!_sent) {
          ...
          const normalized = normalizePayload(raw, form);

          // overwrite payload_json with normalized
          setFieldValue(form, 'payload_json', JSON.stringify(normalized));
          setFieldValue(form, 'total_gross', normalized?.total_gross ?? '');
          setFieldValue(form, 'country_col', currentCountryColKey || '');

          postWebhook(normalized).catch(err => console.warn('[WOMONDO] webhook send failed', err));
    ...
    // capture early even if validation blocks submit
  ...
  async function initialize() {
    ...
    // expose pdf generator if needed
    ...
    window.WOMONDO_getState = () => {
      ...
      return {
        ...
        triggerCodes: TRANSPORT_TRIGGER_CODES,
        existsPrimary: !!map?.has?.(TRANSPORT_PRIMARY),
        existsFallback: !!map?.has?.(TRANSPORT_FALLBACK),
        pricePrimary: map?.get?.(TRANSPORT_PRIMARY),
        priceFallback: map?.get?.(TRANSPORT_FALLBACK),
        autoFees
      };
    };
